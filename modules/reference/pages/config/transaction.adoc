...
|+When backend routing is active, the URL is used by peer servers to contact this server.+
...
|+This attribute is optional. By default, the transaction service stores its recovery logs in files. Alternatively, you can store the logs in an RDBMS. Set this attribute to the ID value for a non-transactional data source where the transaction logs are stored. Do not change the syncQueryTimeoutWithTransactionTimeout value on this data source from the default, which is false.+
...
|+The number of times an expired lease must be seen before peer recovery is attempted+
...
|+Specifies the interval in seconds between attempts to recover from a transient error accessing the transaction service logs.+
...
|+Specifies the maximum number of attempts to recover from a transient error accessing the transaction service logs for an unrecoverable error condition.+
...
|+Specifies whether the remaining time left in a global transaction is propagated to XAResources on enlistment.+
...
|+Specifies whether the transaction manager will stop an application server that is part of a recoveryGroup if an unrecoverable error occurs accessing its own transaction service logs.+
...
|+transactionLogDBName+

|string

|

|+Specifies the name of the database where recovery log tables are created.+
